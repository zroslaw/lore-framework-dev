...
**Artifact-store direction evolved 2026-06-03 (per-repo codex), renamed + purpose-elevated 2026-06-05 — the per-repo DF backbone, skills not agents** (now leading; the context-agent framing above is demoted to a prior step). Per-repo artifacts + narrative context live in a `<repo>-df` mirror (DF = Dark Factory; the backbone the DF runs on) — per-file dirs under a top-level `artifacts/`, lazy creation as a coverage map, `index.md` narrative landing + flat structured aspect subdirs like `ula/` (the `aiqa/` dir level was flattened away). No agent — lr-dev standardizes layout + procedures at the framework level; the per-repo instance is the DF repo + config. This dissolves the context-agent-generator and context-agent open-seams items below. **The DF/ULA design thread is IN PROGRESS (held for continuation) — see `df-ula-design-in-progress.md`.** See `df-per-repo-backbone.md`, `ula-designed-for-multiple-runs.md`, `provenance-header-concept.md`, `ula-artifact-granularity.md`, `ula-narrative-vs-structured-output.md`, `ula-validated-turbo-boost-switcher.md`.
...
- **Aspect-agnostic DF-repo-init skill** — generalize `dev-aiqa-repo-init` into a DF-repo-init that scaffolds `<repo>-df` (with `artifacts/`) + config (today's `aiqa.config.yaml` seed) without committing to one aspect. From the DF-backbone direction.
- **DF-repo migration** — migrate the existing `My-Turbo-Boost-Switcher-aiqa` → `…-df/artifacts/SystemCommands.m/ula/…` + per-file `index.md`. Small, deferred. From `df-per-repo-backbone.md` § Parked.
- **Above-file / repo-level DF layers** — a DF-repo-wide layer above per-file, beside `artifacts/` (`index.md` links "out to repo-level topics"; repo-wide context, cross-cutting aspects, run/task state). Parked. From `df-per-repo-backbone.md` § Parked.
- **Provenance Header schema lock** — lock the self-describing artifact header (`source-sha` blob hash, extensible `config` bag, `ula-version` outside config, drop `source`) into the JSON Schema. From the 2026-06-05 ULA thread. See `provenance-header-concept.md`, `df-ula-design-in-progress.md`.
- **Incremental ULA run algorithm** — detail the SHA-compare + per-unit merge flow (sketched, not detailed). See `ula-designed-for-multiple-runs.md`.
- **ULA artifact granularity decision** — per-file (unit as a field) vs per-unit folders; leaning per-file, user "not sure." See `ula-artifact-granularity.md`.
- **Plugin-side naming ripple** — `dev/aiqa/` module + `/lr:dev-aiqa-repo-init` now out of step with the flattened `ula/` tree and the `-df` rename. From `df-per-repo-backbone.md` § Parked.
- **DF glossary draft** — create `workdir/draft-df-glossary.md` (proposed but held, not created) when the DF/ULA thread resumes. Starter glossary lives in `df-ula-design-in-progress.md`.
- **Open seams** — `index.md` authoring conventions (which narrative belongs file-level vs unit-level) + staleness between non-lr-dev touches; aspect-skill ergonomics (QA-only vs context-only vs combined passes); scenario→code binding; generated-vs-human de-dup. Tracked in `lr-dev-direction.md` § Open decisions and draft §9.
- **DF-repo conventions** — per-file-dir layout (`index.md` + flat aspect subdirs like `ula/<unit>/` under `artifacts/`), lazy-creation-as-coverage-map property, composite-build template, retention/pruning policy. Generalizes the old quality-repo-conventions item (`reports/`/`bugs/`/`scenarios/`/`tests/`/`manifest.yaml`, composite build, manifest schema). May graduate into the DF-repo-init skill. See `df-per-repo-backbone.md`, `quality-repo-architecture.md`.
