...
- **Autonomous agents** (parked vision) — agents as always-on background collaborators with persistent task state, raising for input only when needed. First concrete step: `/lr:spawn-teammate`. See `autonomous-agents-vision.md`, `autonomous-agents-substrate.md`.
...
Workspace holds three repos: **`lore-framework/`** (plugin, VERSION 17, public at github.com/zroslaw/lore-framework), **`lore-framework-dev/`** (this repo — framework-dev agents; a workspace-root sibling at github.com/zroslaw/lore-framework-dev), **`lore-agents/`** (personal agents: tax-advisor, masschallenge-judge). ~85 lore topics.
