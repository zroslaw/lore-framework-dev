...
- **Multi-engine portability (Codex, Cursor)** — major direction, user-raised 2026-07-02; Claude Code stays the major version, goal is Tier-1-parity ports so mixed-engine teams can share one team-shared agent repo. The knowledge substrate (agent repos, `lore-repo.md`, `role.md`, `lore/`, `lore-context.md`, git) is already engine-agnostic — SKILL.md is now an open standard both engines support, AGENTS.md is native to both, MCP ports `lr-wait` unchanged — so the port is mostly packaging, not redesign. The whole port surface is **5 adapter bindings** via a `docs/engines/` convention (framework-root, subagent-spawn, runtime-bounding, memory-file, invocation-syntax); the full Claude-coupling inventory with Tier A/B/C tiering now exists (`claude-coupling-inventory-and-port-tiers.md`, full detail in `workdir/claude-specific-inventory.md`). Dominant shared risk stays: the framework is prose executed by the model — subagent/merge-procedure fidelity on non-Claude models is the hard Tier B nucleus. **Progress (2026-07-04):** the biggest Tier A slice — `${CLAUDE_PLUGIN_ROOT}` → `<framework-root>` by self-location — is now **empirically validated on Claude/haiku, not just planned** (18/19 lifecycle first pass, fan-out clean; `framework-root-self-location-validated.md`), alongside an orthogonal defer-clarity robustness fix (`haiku-ambiguity-detector.md`). Both change sets are **staged in an isolated copy (`LR_FRAMEWORK_DIR`), not yet applied to the live plugin** — deliberate, to land the whole port as one reviewed change set. Next dedicated session is the subagent-adapter pass (Tier B: `docs/engines/` spawn binding + `CLAUDE.md`→`AGENTS.md` with `test_18_init` in lockstep), then apply to the real framework and run the full suite; concrete resume point in `port-landing-next-steps.md`. Manual trial guides `workdir/first-steps-codex.md` (verified) and `workdir/first-steps-cursor.md` (quota-blocked, unconfirmed) also produced. Phase 0.5 groundwork — the lifecycle testing harness (`lore-framework-dev/tests/lifecycle/`, 19/21 on Claude Code) — is what turned "framework is prose" into an observed risk, finding three `agent-boot.md`/`version-check.md` fidelity fixes and validating the port change sets via its `LR_FRAMEWORK_DIR`+`LR_TEST_MODEL=haiku` loop. First engine probes (2026-07-03): codex needs a persistent local install (`codex plugin marketplace/plugin add`, unlike `--plugin-dir`), parsed `marketplace.json` natively, boot-happy-path PASSed; cursor `--plugin-dir` parity confirmed but blocked on account quota. Manual headless-CLI probes must be backgrounded/polled, never foreground-killed. See `lifecycle-testing-harness.md`, `agent-boot-doc-fidelity-fixes.md`, `execution-testing-catches-blind-ambiguity.md`, `haiku-ambiguity-detector.md`, `codex-cli-plugin-loading-findings.md`, `cursor-agent-cli-probe-findings.md`, `headless-cli-smoke-testing-discipline.md`. Anchor: `multi-engine-portability-direction.md`; see `similar-projects-landscape.md` (the positioning case this unlocks — no surveyed competitor federates knowledge across different coding engines).
