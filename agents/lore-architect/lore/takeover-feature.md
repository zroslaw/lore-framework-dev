...
- **Dormant manual archive primitive (v34):** `session-takeover archive` remains available for a future explicitly designed use case, but `/lr:summarize` and `/lr:finalize` do not invoke it. Do not reconnect it to either lifecycle incidentally: any automatic archive feature needs a new design decision, including its privacy and retention boundary.
