...
Framework `main` carries **v36** (shipped 2026-08-09, commit `e5e097b`, tag `lr--v1.36.0`,
pushed). The v36 gates: deterministic suite green (315/315), three TriLens rounds with all findings
applied plus an independent full-diff architect review as the clean-round attestation; **real-engine
lifecycle was explicitly waived by the user at ship time** — the waiver is recorded in
`versioning-release-types.md` (see `gate-waiver-is-a-record.md`). My own Lore corpus is still
essentially legacy; v1 adoption is lazy via merge or explicit via `/lr:groom`. Unrelated
