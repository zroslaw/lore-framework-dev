...
- **Multi-engine portability (Codex, Cursor)** — major direction, user-raised 2026-07-02; Claude Code stays the major version, goal is Tier-1-parity ports so mixed-engine teams can share one team-shared agent repo. The knowledge substrate (agent repos, `lore-repo.md`, `role.md`, `lore/`, `lore-context.md`, git) is already engine-agnostic — SKILL.md is an open standard both engines support, AGENTS.md is native to both, MCP ports `lr-wait` unchanged — so the port is mostly packaging, not redesign. The whole port surface is **5 adapter bindings** via the `docs/engines/` engine-profile convention (framework-root, subagent-spawn, runtime-bounding, memory-file, invocation-syntax) + Boot Step-0 engine selection; the full Claude-coupling inventory with Tier A/B/C tiering exists (`claude-coupling-inventory-and-port-tiers.md`). **Codex is fully ported and validated end-to-end on real `codex exec`** (2026-07-05): the `docs/engines/` build (`lore-framework-codex` sibling, no remote) passes profile selection, framework-root self-location (zero `${CLAUDE_PLUGIN_ROOT}` leak), and native `spawn_agent` fan-out for **both** recall and merge with the host-reads-steps override — the hard Tier-B subagent nucleus is proven, not feared. Codex has a **native in-session multi-agent subsystem** (`spawn_agent`/`wait_agent`, `multi_agent_v1`) — correcting the earlier "no native subagents" read. Sole remaining Codex gap: the `.git`-sandbox commit block (merge work completes on disk; only the commit can't run under the default sandbox). Trust rests on ground-truthing tool use in Codex's rollout logs, not model self-report. Still-open landing work: fold the `docs/engines/` convention back into the canonical `lore-framework`, apply the two still-staged Claude-side change sets (framework-root-full + defer-clarity, validated on haiku and framework-root re-validated on Codex), wire an automated `codex` harness driver, and decide the git-sandbox handling — concrete resume point in `port-landing-next-steps.md`. Cursor: `--plugin-dir` parity confirmed but quota-blocked, unconfirmed. Dominant "framework is prose" risk is empirically **retired for the Codex boot/recall/merge path**; remaining fidelity questions are the un-exercised lifecycle scenarios and Cursor. Phase 0.5 groundwork — the lifecycle testing harness (`lore-framework-dev/tests/lifecycle/`, 19/21 on Claude Code, `LR_FRAMEWORK_DIR`+`LR_TEST_MODEL=haiku` loop) — is what turned "framework is prose" into an observed risk and validated the Claude-side change sets. Anchor: `multi-engine-portability-direction.md`; see `docs-engines-convention.md`, `codex-port-validated-end-to-end.md`, `codex-native-multi-agent-subsystem.md`, `codex-git-sandbox-blocks-dotgit.md`, `codex-testing-methodology.md`, `framework-root-self-location-validated.md`, `claude-coupling-inventory-and-port-tiers.md`, `lifecycle-testing-harness.md`, `codex-cli-plugin-loading-findings.md`, `cursor-agent-cli-probe-findings.md`, `headless-cli-smoke-testing-discipline.md`, `haiku-ambiguity-detector.md`, and `similar-projects-landscape.md` (the positioning case — no surveyed competitor federates knowledge across different coding engines).
...
Workspace holds three canonical repos plus one port build: **`lore-framework/`** (plugin, VERSION 18 — built, commit pending; public at github.com/zroslaw/lore-framework), **`lore-framework-dev/`** (this repo — framework-dev agents; a workspace-root sibling at github.com/zroslaw/lore-framework-dev), **`lore-agents/`** (personal agents: tax-advisor, masschallenge-judge), and **`lore-framework-codex/`** (a no-remote sibling *build* carrying the implemented `docs/engines/` engine-profile port, validated end-to-end on real Codex — to be folded back into canonical `lore-framework`; see `docs-engines-convention.md`, `port-landing-next-steps.md`). The plugin now bundles its first MCP server (`lr-wait`, v18) and carries its first `python3` dependency; dev-only tests live here in `lore-framework-dev/tests/`, not in the plugin — including the multi-engine lifecycle testing harness (`tests/lifecycle/`, 19/21 Tier-1 scenarios passing on Claude Code, gated behind `LR_LIFECYCLE=1`; see `lifecycle-testing-harness.md`). ~99 lore topics.
