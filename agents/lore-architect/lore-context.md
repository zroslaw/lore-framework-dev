...
`AGENTS.md` + a `CLAUDE.md` import stub (`workspace-memory-file-contract.md`). Gates: TriLens round
1 passed, round 2 **did not run** (reviewers died on an account spend limit — not banked as clean,
`a-gate-that-died-is-not-a-gate.md`); deterministic and lifecycle suites deferred by the user.
...
`CLAUDE.md`, so Claude Code sessions here still start with no workspace memory.
