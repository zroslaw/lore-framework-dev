...
- **`being.md` and `create-agent.md` step 8 disagree about who decides registration (v44).** v44's draft makes `/lr:create-agent` register the agent it creates (step 8 runs the Register Agent procedure, step 9 names `/lr:workspace-push`), while `docs/being.md` keeps its opt-out — "do not register a shortcut unless the user asks". The user declined twice to change the being path, so the opt-out stays, but the coupling now rests on a cross-doc inference (step 8 defers to callers; `being.md` step 3 is the caller declining) — exactly the seam where obligations get dropped. The two also disagree about *who decides*: step 8 forbids asking, `being.md` says "unless the user asks". **Do:** settle the decision owner explicitly in one place when v44 resumes. See `create-agent-registers-what-it-creates.md`, `the-terminal-step-is-the-step-that-gets-dropped.md`.
...
- **Keeper hardening findings from real-engine lifecycle verification (2026-07-20).** Gaps surfaced building the `LR_LIFECYCLE_KEEPER=1` Keeper scenarios; filed as findings, none silently patched into `lrb.py` without a doc/test update (they are user-facing schema/design decisions). `cursor` cost-blindness (item 1, `--session-cost-usd` now mandatory for cursor like codex) fixed 2026-07-20 — see `docs/beings.md`, `lrb.py:cmd_engines_add`, `test_lrb.py:TestCliSubprocess.test_engines_add_cursor_kind_requires_session_cost`. Remaining:
  1. **`claude` kind has no `--plugin-dir` mechanism** — `spawn_session` passes `--plugin-dir` only for cursor, so a claude-kind being loads `lr:` skills only if the configured engine `command` itself bakes in `--plugin-dir` (a wrapper script). An operator following `docs/beings.md` literally gets a being that can never load Lore skills. Fix: add an explicit `--plugin-dir` config field for the claude kind, or document the wrapper requirement in `docs/beings.md`. See `engine-kinds-design-decision.md`.
  2. **Test-harness retry for cursor backend flakiness** — cursor-agent's own backend showed transient connection-loss/crash failures on 2026-07-20 (not a Keeper/test bug). Proposed: a capped 1-retry for connection-loss/crash-shaped outcomes scoped to the *test harness* layer (not `lrb.py` — a `timeout`/`crashed` ledger entry is the correct real Keeper signal), with a visible notice when it fires so a real regression isn't masked. See `cursor-agent-real-invocation-contract.md` § Backend reliability.
- **Budget-enforcement leaks found in `lrb.py` code review (2026-07-20).** The daily-usd spawn gate is softer than the "deterministic, unbypassable" framing implies. Deferred deliberately — precise cost accounting isn't the priority at the current PoC phase — but tracked so it doesn't get lost before the Keeper is trusted with real unattended spend:
  1. **Midnight-spanning sessions silently drop cost, in either day** — `_finish` only credits `spent_today_usd` when `started.date() == now.date()` (`lrb.py:1345-1346`). A session starting at 23:58 and finishing at 00:03 has its cost counted nowhere, not misattributed — the daily cap never sees it. Nothing restricts existential-task schedules from firing near midnight.
  2. **No mid-flight USD governor** — the gate (`lrb.py:1051`, `1158`) only checks *finished*-session cost; a single already-running session has no cost ceiling in flight, only a wall-clock kill (up to `MAX_TIMEOUT_MINUTES` = 24h).
  3. **Concurrent-spawn race bypasses the daily cap** — if N tasks for the same being come due in the same 30s tick, each spawn's budget check reads `spent_today_usd` before any prior spawn in that tick has reported cost back, so N concurrent sessions can collectively overshoot `daily_usd` by up to (N−1) sessions' worth. Fix direction for all three: accrue cost by session-*start* day (or split across the boundary), and reserve/pre-commit estimated cost at spawn time rather than only at `_finish`.
- **`docs/wait.md` must state `lr-wait`'s real ceiling and its non-reentrant lock (2026-07-25).** Measured on Claude Code: a single `sleep`/`wait_for_event` call is killed by the engine's MCP idle timeout (`CLAUDE_CODE_MCP_TOOL_IDLE_TIMEOUT`, default 30 min), and the abort leaves the server's single-request lock held with the task id already gone — so `TaskStop` cannot clear it and every later call returns `busy` for the rest of the session. A being or headless agent asking for a multi-hour wait hits both, and the second one silently disables the primitive. Minimum fix: document the ~29-minute practical chunk size, the per-server `timeout` override, and the backgrounded-shell-timer fallback. Better fix: make the busy guard recoverable from an engine-side abort (stale-lock timeout or explicit reset). Not fixed in v30. See `wait-primitive-feature.md` § Long-wait operational limits, `claude-engine-capabilities.md`.
- **Unattended full-permission task execution — trust/security gap (2026-07-20).** Broader and sharper than the existing § Merge Quality / team-shared-lore trust item below: self-scheduling and most real work require `permission_mode: full`, and under team-shared lore, auto-pull means a teammate's push to a being's task content or `being.md` auto-executes headlessly (Bash/Write access, on a schedule) with **no user-triggered review gate** before it runs — unlike `/lr:merge`, which stays user-triggered. Compounded by no notification channel: a bad push to a full-permission being fails silently until someone runs `lrb status`. A being has actual execution capability, not just influence over a future session's reasoning, so this deserves its own threat-model pass rather than folding into the general lore-trust item — options include scoped `--allowedTools` per being instead of blanket full-permission, a pre-execution diff/review gate on task-content changes, and at minimum a push-notification-on-failure channel. Deferred deliberately at the current PoC phase (per user, 2026-07-20) — not urgent, but should not ship past BETA unaddressed. See `lore-beings-mvp-takeover-review.md`, `agent-being-consciousness-substrate-split.md`.
- **The Keeper's login item shows as `python3.14` with a blank icon — `lrb install` should ship a named, iconed wrapper (found 2026-09-05, Intel macOS 26.6.2).** `cmd_install` writes `ProgramArguments[0] = sys.executable` (`lrb.py:1511`), so every macOS Being Keeper appears in **System Settings → General → Login Items & Extensions → App Background Activity** as a bare interpreter name with the generic document icon — the user has no way to tell what that row is or that it is theirs, and it is the first thing a prospective adopter sees of the daemon they just let run at login. **Do:** have `lrb install` lay down a small bundle in `~/.lore-beings/` whose main executable is named `Lore Keeper` and `execv`s into the interpreter, stamp a custom icon on that file, and point `ProgramArguments[0]` at it. **Blocker before shipping:** the strip-signature-then-set-icon step is verified on Intel only; arm64 mandates a signature, so it must be re-tested on Apple Silicon. Until this ships, any hand-applied fix on a machine is reverted by the next `lrb install`. The mechanism, the two rejected alternatives (`AssociatedBundleIdentifiers`, `/usr/bin/open`), and the implementation constraints are canonical in `keeper-login-item-name-and-icon.md` — don't restate them here.
...
- ~~**Reusable multi-lens review skill**~~ — *shipped in framework v30 (2026-07-25) as **core**, not as a DF-module skill.* `parallel-reviewer-fanout-pattern.md` was promoted to `/lr:trilens-loop`; filed here as `lr:dev-review` (draft §6), but the pattern is domain-agnostic (doc sweeps, lore edits, not just code) so it ships in the core namespace with DF/AIQA as a consumer. See `trilens-loop-feature.md`.
...
- **Team lore contribution governance** — design a publish lifecycle distinct from local reflect/merge: proposed branch/worktree change → structural checks → maintainer semantic review → merge. Preserve direct finalization for solo/fully trusted teams; make PR review the small-team default and use protected branches/CODEOWNERS/audit trail for sensitive teams. Review accuracy, scope, safety (secrets/prompt injection), and durable-lore fit. Reuse Git roles/protections before inventing Lore ACLs. See `team-lore-contribution-governance.md`.
...
- **Make "soft skills" an explicit framework entity.** User-raised 2026-06-03. The distinction: **hard skills** are shipped in separate repos and installed deliberately — "hard" because they're hard to change and control. **Soft skills** travel *with the lore agent itself* — behavioral/working-style guidance the agent carries and continuously updates when the user is unhappy or tells it explicitly. They are easy to change because they live next to the agent, not in a distributed package. First concrete instance (the seed for this concept): "follow the user's thinking direction, don't over-think ahead or overtake the direction; suggest small improvements only." Open design questions for later: where soft skills physically live (lore topic? dedicated `soft-skills/` dir? part of `role.md`/`lore-context.md`?), how they're updated (passively via reflection vs. actively on explicit feedback), how they relate to the existing `knowledge-vs-skills-distinction.md` (soft skills look like a *third* category, or a sub-kind of the "skills" axis), and how they're surfaced at boot. Do **not** design now — flagged for a dedicated session. See `knowledge-vs-skills-distinction.md`, `in-flight-skill-teaching-pattern.md`.
  - **Partly resolved in practice (2026-07-05; reshaped in v35).** The concrete need this concept
    was meant to serve is met without a new formal entity: the behavioral guidance ships through the
    regular invokable **`/lr:style`** selector and its internal plain/dialogue/follow components; see
    `style-skills.md`. A boot-loaded soft-skill mechanism (surfacing skills at boot, always-on) was
    **prototyped and deliberately rejected** — the user wanted an explicit selector, not always-on
    machinery (see `skill-request-defaults-to-regular-skill.md`). So the still-open part of this item
    is narrower than originally framed: a *formal entity* is likely unnecessary, but the
    passive-update path (updating a style component's definition via reflection when the user gives
    feedback) remains undesigned.
...
- **`run_matrix.py` exits 0 on non-results.** It exits 0 when the suite refuses to run (`LR_LIFECYCLE` unset, after printing the resolved plan) *and* when module runs fail (a completed matrix summarised `5/27 module runs ok` and still exited 0). Any CI wiring on this runner today is a guaranteed false green. **Do:** exit nonzero on refusal; exit nonzero when any module run failed; give identity-blocked engines a status distinct from `failed`, since they currently render as `failed 0.0s` per module and read as ~18 test failures when they are *two engines that never started*. See `lifecycle-harness-exit-code-is-not-a-verdict.md`, `a-gate-that-died-is-not-a-gate.md`.
- **`test_05` / `test_08` / `test_12` are structurally flaky.** Each asserts the end state of a long model-driven chain and fails at whatever step the model stopped at — `test_05` failed at three different assertions across three runs, the exact distribution its own source comment documents. A test that moves between runs certifies nothing. **Do:** restructure to assert per-step, or accept them as non-gating. `test_08` additionally reads the wrong capture surface (`transcript-vs-final-message-assertions.md`). See `triage-a-red-module-against-its-own-history.md`.
- **No test covers `preflight --agent-dir`'s upward search (v44).** The new resolution was hand-verified across five invocation shapes, which is not a gate. **Do:** add a deterministic test, proved red against v43 (`prove-a-new-test-red-against-the-previous-tag.md`).
- **Codex/Cursor gate coverage needs local install changes — user decision.** On this machine a worktree-based run passes identity on Claude only: Codex's marketplace source in `~/.codex/config.toml` points at the main checkout, and an installed Cursor v42 tree can outrank `--plugin-dir`. Clearing them means repointing Codex's source at the worktree and moving `~/.cursor/plugins` aside, then re-verifying identity *after* the move. Not to be done silently mid-gate. See `lifecycle-harness-plugin-identity-unverified.md`.
