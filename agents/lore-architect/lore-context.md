...
- **Autonomous agents / Lore Beings** — agents as always-on background collaborators with persistent task state, raising for input only when needed. Concrete steps taken: `/lr:spawn-teammate` (multi-agent substrate, v10) and the v18 **`lr-wait`** primitive — the first *inbound-signal* step: an agent blocks on an event and an external actor (cron/CI/webhook/human, via `lr-emit`) wakes it with text. **The beings design is settled and build-ready (2026-07-19, design dialogue, then a same-day user-directed MVP simplification pass): the module is _Lore Beings_.** A being is an ordinary lore agent plus a `being.md` descriptor declaring a lifecycle of **existential tasks** (morning wakeup: plan the day, schedule intraday sessions; night sleep: reflect/finalize/consolidate — converges with the "sleep pass" housekeeping job; weekly self-reflection). The **Being Keeper** — a deterministic machine-level daemon under launchd (never an LLM, never personified) — spawns headless engine sessions on schedule, enforces the budget/kill-switch, and accepts being-initiated schedule requests via an `lr-emit`-style outbox. **MVP is CLI-only** (`lrb`; `lrb-*` skills deferred, namespace reserved); engines are explicit user config, never auto-detected; no framework-canonical task content (per-task prompts are agent-level). The minimalism directive (`feedback-mvp-minimalism.md`) drove a simplification pass that cut the model-tier system, the `autonomy:` field, per-session USD caps, `timezone:`, and the deferred skills — each with a named reintroduction trigger; budget enforcement is now exactly two mechanisms, daily-USD spawn gate + per-task wall-clock kill. That pass surfaced a new named principle, **`unenforceable-caps-are-prompt-theater.md`** (a limit only belongs in the substrate contract if the substrate can enforce it), sharpening the governing **consciousness (LLM) / substrate (deterministic code) split** — which also resolved the vision's long-open "cost / loop safety" question for free. Full agreed design in `workdir/draft-lore-beings.md`; first being is the **Chronicler** (zero-blast-radius test being; MVP = "the Chronicler lives for a week"). Nothing shipped yet. Anchor: `lore-beings-design.md`; see `agent-being-consciousness-substrate-split.md`, `unenforceable-caps-are-prompt-theater.md`, `feedback-mvp-minimalism.md`, `autonomous-agents-vision.md`, `autonomous-agents-substrate.md`, `wait-primitive-feature.md`, `framework-improvements-backlog.md` § Major Directions § Autonomous Agents / Lore Beings.
