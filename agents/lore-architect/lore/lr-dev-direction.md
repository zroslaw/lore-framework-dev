...
**Disposition — DF inside `lr` is deliberate, not a defect (affirmed 2026-06-13 architecture review).** The module living inside the `lr` plugin rather than as a standalone plugin is a *chosen current decision*, not architectural debt — do not re-raise it as a problem. DF *may* graduate to a sibling plugin later; the `df-` skill prefix (and the `df/` module subtree) are already pre-factored so the split would be mechanical. Treat the split as a known, open future option, not a pending fix. See `architecture-review-dispositions.md` for the full review triage; `framework-improvements-backlog.md` § Architecture-Review Follow-Ups for the separately-deferred simplification/subtraction theme.
