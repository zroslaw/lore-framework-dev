...
v36** ("Lore structure and grooming" — Lore v1 taxonomy, boot maps, `/lr:groom`; full entry in
`versioning-release-types.md`, don't reconstruct it from here). Each agent repo stamps that
...
- **`/lr:workspace-status`** (v37) — read-only diagnosis of the *workspace* layer (git state, descriptor drift, memory-file contract, child-repo hygiene), findings S1–S15 each naming its fix. The three-way split all three docs now state: status = this workspace's git/descriptor state, check = content consistency inside agent repos, doctor = engine/plugin runtime. See `workspace-lifecycle-four-commands.md`.
...
Framework `main` carries **v36** shipped (2026-08-09, `e5e097b`, tag `lr--v1.36.0`, pushed); its
per-gate record — including the user-waived real-engine lifecycle — is in
`versioning-release-types.md`. My own Lore corpus is still essentially legacy (216 of 228 files);
v1 adoption is lazy via merge or explicit via `/lr:groom`, and **`lore-context.md` is over its 10K
v1 target** — a grooming pass, not a merge, is the fix. Unrelated uncommitted WIP may sit on these
checkouts; do not sweep it into lore-finalize commits (`git add agents/` only).
...
**v37 is prepared but unshipped** — committed locally on framework `main` as `d2190b5`,
deliberately **not tagged and not pushed** at user instruction. It completes the workspace layer
(`workspace-lifecycle-four-commands.md`) and replaces the per-engine memory file with
`AGENTS.md` + a `CLAUDE.md` import stub (`workspace-memory-file-contract.md`), and carries a
Codex-only shortcut relocation (`codex-shortcuts-are-workspace-local.md`). Gates, per the full record
in `versioning-release-types.md`: a three-round, nine-lens TriLens over the whole release
(all lenses reporting; rounds 2 and 3 each caught a defect introduced by the previous round's own fix
— `a-fix-is-a-change-and-changes-need-review.md`) plus a green 387-test deterministic suite;
**lifecycle and quality suites still not run**, and S1–S14 have no unit coverage.
`lore-framework-dev`'s own `lore-repo.md` still stamps `36` — `/lr:update` bumps it. **This
workspace has not been dogfooded onto v37**: its `AGENTS.md` is still marker-format with no
`CLAUDE.md`, so Claude Code sessions here still start with no workspace memory — and per the
execution-first ordering above, that dogfood is the cheapest remaining gate, not the last one.
