...
- **Engine invocation is dispatched by a small closed `kind` enum** (`ENGINE_KINDS = ("claude", "codex")`), not a config-time invocation template — the second engine (`codex`) shipped in v28 and its contract differs from `claude` on every axis (subcommand vs flag, JSONL vs single JSON, and no USD cost field), so cost-blind kinds charge a mandatory flat `--session-cost-usd` per finished session. See `engine-kinds-design-decision.md` and the underlying empirical contract in `codex-exec-real-invocation-contract.md`.
