...
**Registered-shortcut drift (18, extended 2026-07-28):** check #18 now also asserts that a generated per-agent shortcut's bootstrap body is a **single unwrapped line**. Byte-exact `current` classification makes template whitespace part of the contract, and Claude Code renders a slash command's description from the file's first line — a wrapped bootstrap is never `current` and displays as a fragment. Paired with `test_bootstrap_body_is_a_single_line`. See `template-whitespace-is-contract-under-byte-exact-idempotency.md`, `registered-shortcuts-are-framework-owned.md`.
