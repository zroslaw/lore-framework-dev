...
**Safety-gate discipline:** dirty-tree gates protect *writes*, not reads. When adding a "skip if dirty" gate to a framework operation, articulate the collision scenario explicitly. If the operation only advances refs / is naturally idempotent / git handles the conflict cleanly, no gate — it becomes friction without safety. v13's `auto-pull.md` deliberately has no dirty gate (it's a read); `version-check.md` does (it writes). See **`dirty-tree-gates-write-vs-read-distinction.md`**, also adjacent to `tooling-cwd-safety.md` ("name what the gate is protecting against" applied to CWD).
...
`lore-framework/VERSION` is the single source of truth (**currently `13`**). Each repo stamps this version in `lore-repo.md` frontmatter — one identifier per repo, no per-agent stamps.
