...
- **Autonomous agents / Lore Beings** — agents as always-on background collaborators with persistent task state, raising for input only when needed. Concrete steps taken: `/lr:spawn-teammate` (multi-agent substrate, v10) and the v18 **`lr-wait`** primitive — the first *inbound-signal* step: an agent blocks on an event and an external actor (cron/CI/webhook/human, via `lr-emit`) wakes it with text. **The beings design is settled (2026-07-19, design dialogue): the module is _Lore Beings_.** A being is an ordinary lore agent plus a `being.md` descriptor declaring a lifecycle of **existential tasks** (morning wakeup: plan the day, schedule intraday sessions; night sleep: reflect/finalize/consolidate — converges with the "sleep pass" housekeeping job; weekly self-reflection). The **Being Keeper** — a deterministic machine-level daemon under launchd (never an LLM, never personified) — spawns headless engine sessions on schedule, enforces dollar-primary budgets/kill-switch, and accepts being-initiated schedule requests via an `lr-emit`-style outbox. CLI `lrb`, skills `lrb-*`; engines are explicit user config, never auto-detected; no framework-canonical task content (per-task prompts are agent-level). Governing principle: **consciousness (LLM) / substrate (deterministic code) split** — it also resolved the vision's long-open "cost / loop safety" question for free. Full agreed design in `workdir/draft-lore-beings.md`; first being is the **Chronicler** (zero-blast-radius test being; MVP = "the Chronicler lives for a week"). Nothing shipped yet. Anchor: `lore-beings-design.md`; see `agent-being-consciousness-substrate-split.md`, `autonomous-agents-vision.md`, `autonomous-agents-substrate.md`, `wait-primitive-feature.md`, `framework-improvements-backlog.md` § Major Directions § Autonomous Agents / Lore Beings.
