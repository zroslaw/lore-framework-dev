...
The autonomous-agents direction's beings shape has a **settled design** as of 2026-07-19, agreed in a full dialogue-mode session with the user, then reworked the same day by a user-directed **MVP simplification pass** (directive: keep the initial design as simple as possible; introduce nothing the MVP doesn't exercise — see `feedback-mvp-minimalism.md`). The complete design lives in **`workdir/draft-lore-beings.md`** (which supersedes and replaced the same-day `draft-agent-beings.md`). **Status:** v28 shipped the Lore Beings substrate as BETA (`lore-framework/scripts/lrb.py`, `docs/beings.md`, deterministic tests in `lore-framework-dev/tests/test_lrb.py`); active v29 work adds `/lr:being` as the user-facing command surface plus `lrb validate`/`lrb logs`. The v28 build was real-engine-validated across engine kinds, independently reviewed and hardened across review passes — see the draft's **§16 "Build & hardening notes"**, `lore-beings-mvp-takeover-review.md`, `engine-kinds-design-decision.md`, and `macos-ps-o-multi-field-single-line.md`. **Keeper-specific real-engine lifecycle coverage now exists as a separate suite**: `tests/lifecycle_beings/keeper_harness.py`, `test_lrb_lifecycle.py`, and `run_matrix.py`, gated behind `LR_LIFECYCLE_BEINGS=1` because it has higher blast radius than the standard lifecycle suite. See `lifecycle-testing-harness.md` § Keeper coverage and `tests/testing-strategy.md`.
...
- User-facing command **`/lr:being`** (v29) is the single entry point for status/help/init/create/validate/logs/Keeper/engine/workspace/pause/resume. Substrate CLI **`lrb`** remains deterministic and is not the normal user UX. Machine home `~/.lore-beings/`; workspace state `<workspace>/.lr-beings/` (gitignored).
...
- `lifecycle-testing-harness.md` § Keeper coverage — the `LR_LIFECYCLE_BEINGS=1` real-engine Keeper scenarios
