...
- **Autonomous agents / Lore Beings** — agents as always-on background collaborators with persistent task state, raising for input only when needed. Concrete steps taken: `/lr:spawn-teammate` (multi-agent substrate, v10) and the v18 **`lr-wait`** primitive — the first *inbound-signal* step: an agent blocks on an event and an external actor (cron/CI/webhook/human, via `lr-emit`) wakes it with text. **The beings design is settled (2026-07-19, design dialogue, then a same-day user-directed MVP simplification pass): the module is _Lore Beings_.** A being is an ordinary lore agent plus a `being.md` descriptor declaring a lifecycle of **existential tasks**; the **Being Keeper** is a deterministic machine-level daemon under launchd (never an LLM, never personified) that spawns headless engine sessions on schedule, enforces the budget/kill-switch, and accepts being-initiated schedule requests via an `lr-emit`-style outbox. **MVP is CLI-only** (`lrb`; `lrb-*` skills deferred, namespace reserved); engines are explicit user config, never auto-detected; no framework-canonical task content. The minimalism directive (`feedback-mvp-minimalism.md`) cut the model-tier system, `autonomy:`, per-session USD caps, `timezone:`, and deferred skills; budget enforcement is exactly daily-USD spawn gate + per-task wall-clock kill. **Shipped as v28 (BETA), release-committed but not yet pushed** (2026-07-20): Claude Code built and cross-model-reviewed the MVP, Codex took over for a third review (local worktree checkpoints, `2badbdb` framework / `961ac22` dev, 70+129 tests green — but run under a sandbox that blocked `ps`, so the PID-identity confirmed-match/confirmed-mismatch branches never actually executed), then a fourth pass on real unsandboxed macOS immediately caught a real bug and added the second engine kind: both worktrees merged into main and released as v28 (`44bc57d`, on top of the PID-identity fix and the `codex` engine kind). Codex did **not** have the raw exported Claude Code transcript; the preserved Claude knowledge is the existing build-note summary in `workdir/draft-lore-beings.md` §16. **Remaining gate before push: the full `LR_LIFECYCLE=1` suite** (this session ran unit tests + targeted e2e smoke, not the same gate). Still not installed as a persistent `--launchd` daemon on any real machine. Open gap: self-scheduling/writes need an explicit headless permission policy (full permission mode vs. future scoped allowed-tools); safe default denies Bash/Write. First being is the **Chronicler** (zero-blast-radius test being; MVP = "the Chronicler lives for a week"). Anchor: `lore-beings-design.md`; see `lore-beings-mvp-takeover-review.md`, `engine-kinds-design-decision.md`, `codex-exec-real-invocation-contract.md`, `macos-ps-o-multi-field-single-line.md`, `agent-being-consciousness-substrate-split.md`, `unenforceable-caps-are-prompt-theater.md`, `feedback-mvp-minimalism.md`, `autonomous-agents-vision.md`, `autonomous-agents-substrate.md`, `wait-primitive-feature.md`, `framework-improvements-backlog.md` § Major Directions § Autonomous Agents / Lore Beings.
...
(Lore Beings, BETA) is release-committed on top of v27 (`44bc57d`) but not yet pushed** — full
lifecycle suite still owed before push; see the Autonomous Agents / Lore Beings entry above and
`lore-beings-design.md`. Keep release notes truthful about completed vs partial gates. See
`versioning-release-types.md`, `release-notes/27.md` and `release-notes/28.md` (plugin tree),
`lifecycle-testing-harness.md`, `lore-agents-product-name.md`, `onboarding-funnel-team-join-path.md`.
