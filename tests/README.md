...
## Lore Beings lifecycle scenarios (Layer 3, Being Keeper)

`lifecycle_beings/` is a separate lifecycle suite for Lore Beings. It is not part of
`tests/lifecycle/run_matrix.py` and does not run with `LR_LIFECYCLE=1`.

- **`lifecycle_beings/test_lrb_lifecycle.py`** + **`lifecycle_beings/keeper_harness.py`**
  ...
  **Gated separately from `LR_LIFECYCLE`: `LR_LIFECYCLE_BEINGS=1`.** Keeper scenarios have a
  ...
  LR_LIFECYCLE_BEINGS=1 LR_ENGINE=codex  python3 tests/lifecycle_beings/test_lrb_lifecycle.py -v -k a2
  LR_LIFECYCLE_BEINGS=1 LR_ENGINE=cursor python3 tests/lifecycle_beings/test_lrb_lifecycle.py -v -k a3
