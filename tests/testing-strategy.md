...
### Layer 3b: Lore Beings Lifecycle Tests

Lore Beings lifecycle tests cover the Being Keeper runtime (`lrb`) where real
process behavior matters. They are a separate suite, not part of the standard
`tests/lifecycle/` runner, and are gated with `LR_LIFECYCLE_BEINGS=1` because
some scenarios spawn real process trees or daemon subprocesses.

Main files:

- [`lifecycle_beings/run_matrix.py`](lifecycle_beings/run_matrix.py)
- [`lifecycle_beings/test_lrb_lifecycle.py`](lifecycle_beings/test_lrb_lifecycle.py)
- [`lifecycle_beings/keeper_harness.py`](lifecycle_beings/keeper_harness.py)
...
The Lore Beings runner records durable results under
`tests/lifecycle_beings/results/`. Use `--dry-run` freely; do not set
`LR_LIFECYCLE_BEINGS=1` unless intentionally spending model/API budget on the
BETA suite.
...
| Lore Beings lifecycle | real engine cost + process risk | high | targeted and Beings-release-gate only |
...
3. Lore Beings lifecycle matrix evidence when the release touches Lore Beings or the Keeper.
4. Quality benchmark evidence for the regular matrix.
5. Durable summaries saved under the relevant `results/` directory.
6. Release notes updated with the quality benchmark report block when quality evidence is part of
   the release gate.
...
Lore Beings lifecycle matrix runs write under:
...
Add a Lore Beings lifecycle scenario when:

- the behavior belongs to the Being Keeper (`lrb`) runtime
- the proof requires real process trees, daemon lifecycle, PID identity, or real engine result shape
- the test needs the higher-blast-radius `LR_LIFECYCLE_BEINGS=1` gate
