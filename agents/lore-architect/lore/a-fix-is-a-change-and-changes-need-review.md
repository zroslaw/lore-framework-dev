...
**Second and third occurrences, and the analysis.** v38 and then v39 repeated the pattern exactly —
each round after the first found a defect created by the previous round's fix (v39 across four fix
rounds, counting its substitute round). Three releases is more than enough to generalise, and the
generalisation is sharper than this topic's rule:
