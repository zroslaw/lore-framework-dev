...
### Autonomous Agents / Agent Beings
...
- **Agent Beings — existential tasks + supervisor ("agent OS") (2026-07-19 brainstorm capture).** User-articulated elaboration of autonomy-in-time: an agent is a **being** with a declared lifecycle of **existential tasks** — e.g. *morning wakeup* (boot, pull, review backlog, plan the day, schedule intraday work sessions), *night sleep* (reflect, finalize, consolidate — converges with the "sleep pass" § Knowledge Quality), *weekly self-reflection* (long-range self-improvement). A **supervisor** — deterministic daemon, not LLM — is the "unconscious substrate / operating system": reads per-agent **being definitions** (existential task list, schedules, budgets, autonomy level), spawns headless engine sessions on schedule, monitors status/logs/token spend, enforces budget caps, and accepts **schedule requests from agents** (file-outbox channel, symmetric to `lr-emit`) so an agent can schedule its own future sessions. `lr-wait` remains the *synchronous, in-session* wait primitive; the supervisor owns *between-session* existence. Status: brainstorm capture — promote to `workdir/draft-agent-beings.md` in a dedicated design session before building. See `autonomous-agents-vision.md`, `autonomous-agents-substrate.md`, `wait-primitive-feature.md`.
